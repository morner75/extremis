...
angcdf.EL <- function(Y, tau = 0.95, method = "euclidean", raw = TRUE)
    UseMethod("angcdf.EL")

angcdf.EL.default <- function(Y, tau = 0.95, method = "euclidean", 
                              raw = TRUE) {
    ...
    outputs <- list(H = H, p = p, w = w, Y = Y)
    class(outputs) <- "angcdf.EL"
    return(outputs)
}

plot.angcdf.EL <- function(x, ylim = NULL, original = TRUE, main = "", ...) 
